...
uint8_t DATA_BUF_RES[ETX_OTA_PACKET_MAX_SIZE];
...
{
  bool is_ack  = false;
  memset(DATA_BUF_RES, 0, ETX_OTA_PACKET_MAX_SIZE);

  uint16_t len =  RS232_PollComport( comport, DATA_BUF_RES, sizeof(ETX_OTA_RESP_));
  //printf("is_ack_resp_received len = %d  \r\n",len);

  if( len > 0 )
  {
    ETX_OTA_RESP_ *resp = (ETX_OTA_RESP_*) DATA_BUF_RES;
    if( resp->packet_type == ETX_OTA_PACKET_TYPE_RESPONSE )
    {
      // printf(" resp->packet_type == ETX_OTA_PACKET_TYPE_RESPONSE \r\n");
      //TODO: Add CRC check
...
int send_ota_start(int comport)
{
  ...
  //send OTA START
  printf("Send OTA Start len %d \r\n",len);
  for(int i = 0; i < len; i++)
  {
    delay(40000);
    if(SEND_START==1 || is_ack_resp_received( comport ))
    {
        SEND_START = 0;
        printf("Send data DATA_BUF %d = %d Done \r\n",i,DATA_BUF[i]  );
